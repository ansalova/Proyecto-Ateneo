...
import { CheckCircle, Clock, XCircle, Home, MessageSquare, Mail, AlertTriangle } from 'lucide-react';
...
{emailSent ? (
                    <> <Mail size={14} className="inline mr-1" />Se envió un correo a <strong>{user.email}</strong></>
                  ) : (
                    <> <AlertTriangle size={14} className="inline mr-1" />No se pudo enviar el correo a <strong>{user.email}</strong>{emailError ? `: ${emailError}` : ''}</>
                  )}
                </p>
              )}
            </div>
