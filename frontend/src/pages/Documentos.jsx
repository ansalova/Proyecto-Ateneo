...
import { AlertTriangle, Lock, File, Link2, Check, Package, XCircle } from 'lucide-react'
...
export default function Documentos() {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      if (err.response?.status === 401) {
        errorMsg = (<><AlertTriangle size={14} className="inline mr-1" />Tu sesión expiró. Por favor inicia sesión de nuevo.</>);
      } else if (err.response?.status === 403) {
        errorMsg = (<><Lock size={14} className="inline mr-1" />No tienes permiso para ver estos documentos.</>);
      } else if (err.response?.status === 500) {
        errorMsg = (<><AlertTriangle size={14} className="inline mr-1" />Error del servidor. Por favor intenta más tarde.</>);
      }
  ...
  const handleCreateDocument = async (e) => {
    ...
    if (!formData.title.trim()) {
      setToast({ type: 'warning', message: (<><AlertTriangle size={14} className="inline mr-1" />El título del documento es obligatorio</>) })
      return
    }
    ...
    if (!hasURL && !hasFile) {
      setToast({ type: 'warning', message: (<><File size={14} className="inline mr-1" />Debes subir un archivo o proporcionar una URL</>) })
      return
    }
    ...
    if (hasURL && !hasFile) {
      try {
        ...
        setToast({ type: 'warning', message: (<><Link2 size={14} className="inline mr-1" />Ingresa una URL válida (ej: https://ejemplo.com/archivo.pdf)</>) })
        return
      }
    }
    ...
    try {
      ...
      setShowForm(false)
      setError('')
      setToast({ type: 'success', message: (<><Check size={14} className="inline mr-1" />Documento publicado exitosamente</>) })
      fetchDocuments()
    } catch (err) {
                  ...
                  onChange={(e) => {
                    ...
                    if (f) {
                      ...
                      if (fileSizeMB > 5) {
                        setToast({ 
                          type: 'error', 
                          message: (<><Package size={14} className="inline mr-1" />El archivo es muy grande (${fileSizeMB.toFixed(2)}MB). Máximo permitido: 5 MB.</>)
                        })
                        return
                      }
